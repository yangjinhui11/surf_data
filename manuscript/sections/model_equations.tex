...
\end{equation}
where $f_i$ is the fractional area of tile $i$.  This tiled structure is
...
closure before the land update is invoked.  In compact form the
surface-layer tendency passed to SURF can be written as
\begin{equation}
  \Delta \phi_0 = \mathcal{V}_{\phi}
  (\phi_a, \phi_s, z_0, C_D, C_H, C_E, |\mathbf{V}|, \Delta t),
  \qquad \phi \in \{u, v, T, q\},
\label{eq:model-vdf}
\end{equation}
where $\mathcal{V}_{\phi}$ denotes the vertical-diffusion closure used by the
IFS physics package.  This closure is part of the offline PyTorch model only.
...
bounded by a vegetation-dependent maximum storage,
\begin{equation}
  0 \le W_{\rm skin} \le W_{\rm skin}^{\max}(LAI, f_{\rm veg}),
\label{eq:model-wskinmax}
\end{equation}
...
conductivity, and $S_k$ contains surface and phase-change source terms.  In
finite-volume form,
\begin{equation}
  C_k \Delta z_k \frac{T_k^{n+1}-T_k^n}{\Delta t}
  = F_{k-1/2}^{n+1} - F_{k+1/2}^{n+1} + S_k\Delta z_k,
\label{eq:model-soilfv}
\end{equation}
with conductive flux
\begin{equation}
  F_{k+1/2} = \lambda_{k+1/2}
  \frac{T_{k+1}-T_k}{z_{k+1}-z_k}.
\label{eq:model-soilflux}
\end{equation}
The implicit tridiagonal solve and the layer geometry are preserved from the
...
The prognostic water mass is
\begin{equation}
  W_k = \rho_w \Delta z_k \theta_k,
\label{eq:model-wmass}
\end{equation}
and is bounded by soil-type-dependent residual, wilting-point, field-capacity,
...
temperature, density, and albedo.  Snow water equivalent obeys
\begin{equation}
...
$M_{\rm snow}$ is melt.  Snow temperature is advanced by an energy balance
coupled to surface turbulent fluxes, radiation, and the soil heat flux,
\begin{equation}
  C_{\rm snow} \frac{{\rm d}T_{\rm snow}}{{\rm d}t}
  =
  R_{n,\rm snow} - H_{\rm snow} - L_sE_{\rm snow}
  - G_{\rm snow} - L_f M_{\rm snow}.
\label{eq:model-tsnow}
\end{equation}
Snow density evolves through compaction and fresh-snow updates, while snow
